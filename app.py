...
2. Make sure you have data/adult.data.csv
...
5. API endpoints available:
   ...
   - http://localhost:5000/api/race-distribution
   - http://localhost:5000/api/education-breakdown
   - http://localhost:5000/api/country-analysis
   - http://localhost:5000/api/age-distribution
...
@app.route('/path')    → Creates URL endpoint
render_template()      → Sends HTML file
jsonify()             → Converts dict to JSON
request.files         → Access uploaded files
request.get_json()    → Get JSON from POST request
CORS                  → Allow frontend API calls
debug=True            → Development mode

THAT'S IT! You now understand Flask! 🎉
"""